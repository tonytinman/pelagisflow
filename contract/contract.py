...
# ==== DQ EXTENSIONS ====
    ...
    def cleansing_rules(self):
        ...
        for rule in self.raw_quality_rules:
            ...
            def cols_for(allowed=None):
                cols = self._expand_columns(explicit_cols, allowed)
                return self._validate_columns(cols, name)

            if name == "trim_string_fields":
                for col in cols_for(["string"]):
                    rules.append({"rule": "trim", "column": col})
                ...
                for col in cols_for(["string"]):
                    rules.append({"rule": "lower", "column": col})

            elif name == "nullify_empty_strings":
                for col in cols_for(["string"]):
                    rules.append({"rule": "nullify_empty_strings", "column": col})
                ...
                for col in cols_for(None):
                    rules.append({"rule": "normalize_boolean_values", "column": col})
    ...
    def quality_rules(self):
        ...
        for rule in self.raw_quality_rules:
            ...
            weight = {"critical":5,"warning":3,"info":1}.get(sev,1)
            explicit_cols = rule.get("columns", [])

            cols = self._validate_columns(self._expand_columns(explicit_cols), name)
            ...
            if name in simple:
                for col in cols:
                    r = {"rule": simple[name], "column": col, "weight": weight}
                    if name=="regex": r["pattern"]=rule["pattern"]
                    if name=="allowed_values": r["values"]=rule["values"]
                    if name in ("min_length","max_length","min","max"): r["value"]=rule["value"]
                    if name=="between": r["min"]=rule["min"]; r["max"]=rule["max"]
                    if name=="is_date": r["format"]=rule.get("format","yyyy-MM-dd")
                    rules.append(r)
                continue

            if name=="composite_unique":
                if len(cols)>=2:
                    rules.append({"rule":"composite_unique","columns":cols,"weight":weight})
                continue

            if name in ("conditional","conditional_required"):
                rules.append({
                    "rule": "conditional",
